...
def sud2sat():
    # Read in the input file of all the hard puzzles and then parse into the separate ones
    # Replace all to periods
    hard_suds = sys.stdin.read()
    hard_suds = hard_suds.replace('0', '.').replace('*', '.').replace('?', '.').split('\n')
    if hard_suds[-1] == '':
        hard_suds = hard_suds[:-1]


    for index, line in enumerate(hard_suds):
        # Remove any whitespace
        sud_input = ''.join(line.split())
        # Back to rows
        ...
        file = open("puzzle" + str(index) + ".cnf", "w+")
        file.write("p cnf " + str(729) + " " + str(number_of_clauses) + "\n" + final_res)
        file.close()
